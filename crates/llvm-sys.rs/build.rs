...
// clydec-fix: "component selction" (as done by rustc), limit the number of llvm components to
// link, in order to:
//  - improve link-time
//  - get smaller binary size
//  - fix rust "archive breaking" and resulting link errors on windows
...
fn get_link_libraries(
    ...
    fn get_link_libraries_impl(
        ...
        let mut xxx = vec!["--libnames", link_arg];
        xxx.extend(components);
        llvm_config_ex(llvm_config_path, &xxx)
    }
    ...
    for kind in preferences {
        match get_link_libraries_impl(llvm_config_path, kind, components) {
...
fn main() {
    ...
    let components = llvm_config_ex(&llvm_config_path, ["--components"]).unwrap();
    ...
    let (kind, libs) = get_link_libraries(&llvm_config_path, &preferences, &components);
    for name in libs {
