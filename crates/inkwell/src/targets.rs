...
impl Target {
    ...
    // clydec-fix: remove `initialize_native` and `initialize_all` in order to compile with
    // feature = "llvm-sys/disable-alltargets-init", which now causes compile errors due to
    // component selection in llvm-sys.rs/build.rs
